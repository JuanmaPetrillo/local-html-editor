...
import {
  ...
  editHeadingTextInModel, addTextBlockToSlide, deleteFirstTagInSlide
...
if (model.slides.length !== 3) throw new Error('slide count mismatch');
...
for (const cls of ['topnav', 'slides-area', 'slide-inner', 'botnav', 'progress-track', 'progress-fill', 'slide active']) if (!exported.includes(cls)) throw new Error(`expected class missing: ${cls}`);
...
if (!restored || restored.slides.length !== 3) throw new Error('project save/open failed');
